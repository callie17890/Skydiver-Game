...
Skydiver.level2 = function() {};
Skydiver.level2.prototype = {
	preload: function() {
		this.load.image('girl', 'images/girl.png');
		this.load.image('background', 'images/2.jpg');
	...
	create: function() {
    	...
    	//stars.setAll('girl.x', 'girl.y');
    	//stars.setAll('girl.y', 0.5);
    	//stars.setAll('outOfBoundsKill', true);
    	//stars.setAll('checkWorldBounds', true);
		
		//creating a timer, every 3 seconds creates new coin
		this.time.events.loop(Phaser.Timer.SECOND * 3, this.create_coin, this);
		this.time.events.loop(Phaser.Timer.SECOND * 0.4, this.create_bomb, this);

		//level  text
		level_text = this.add.text(10, 10,'Level 2', { font: '25px Century Gothic', fill: 'white' });
		//score text
		score_text = this.add.text(10, 40,'Score: 0', { font: '25px Century Gothic', fill: 'white' });
		this.add.text(10, 570,'Coins needed for next level: 10', { font: '20px Century Gothic', fill: 'white' });
