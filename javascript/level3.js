...
Skydiver.level3 = function() {};
Skydiver.level3.prototype = {
	preload: function() {
		this.load.image('girl', 'images/girl.png');
		this.load.image('background', 'images/3.jpg');
	...
	create: function() {
    	...
    	// stars.setAll('girl.x', 0.5);
    	// stars.setAll('girl.y', 0.5);
    	// stars.setAll('checkWorldBounds', true);
		
		//creating a timer, every 3 seconds creates new coin
		this.time.events.loop(Phaser.Timer.SECOND * 4, this.create_coin, this);
		this.time.events.loop(Phaser.Timer.SECOND * 0.3, this.create_bomb, this);

		//level text
		level_text = this.add.text(10, 10,'Level 3', { font: '25px Century Gothic', fill: 'white' });
		//score text
		score_text = this.add.text(10, 40,'Score: 0', { font: '25px Century Gothic', fill: 'white' });
		this.add.text(10, 570,'Coins needed to win: 15', { font: '20px Century Gothic', fill: 'white' });
